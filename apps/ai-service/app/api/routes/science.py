...
async def _guard_conversation(
    ...
    try:
        await to_thread.run_sync(
            science_service.ensure_access, conversation_id, principal.user_id
        )
    except KeyError:
...
async def science_chat(
    req: ScienceStreamRequest, principal: Principal = CurrentUser
) -> StreamingResponse:
...
async def science_history(
    ...
    try:
        return await to_thread.run_sync(
            science_service.get_history, conversation_id, principal.user_id
        )
    except KeyError:
        raise HTTPException(status_code=404, detail="会话不存在") from None
...
async def science_conversations(
    ...
    return await to_thread.run_sync(
        science_service.list_conversations, principal.user_id
    )
