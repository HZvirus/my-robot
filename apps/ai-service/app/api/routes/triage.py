...
async def _guard_conversation(conversation_id: str | None, user_id: str) -> None:
    ...
    try:
        await to_thread.run_sync(
            triage_service.ensure_access, conversation_id, user_id
        )
    except KeyError:
...
async def triage_chat(
    req: TriageRequest, user_id: str = CurrentUser
) -> StreamingResponse:
...
async def triage_history(
    ...
    try:
        return await to_thread.run_sync(
            triage_service.get_history, conversation_id, user_id
        )
    except KeyError:
        raise HTTPException(status_code=404, detail='会话不存在') from None
...
async def triage_conversations(
    ...
    return await to_thread.run_sync(triage_service.list_conversations, user_id)
