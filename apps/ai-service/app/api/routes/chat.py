...
async def _guard_conversation(
    ...
    try:
        await to_thread.run_sync(
            chat_service.ensure_access, conversation_id, principal.user_id
        )
    except KeyError:
...
async def chat(req: ChatRequest, principal: Principal = CurrentUser) -> ChatResponse:
...
async def chat_stream(
    req: ChatStreamRequest, principal: Principal = CurrentUser
) -> StreamingResponse:
...
async def chat_history(
    ...
    try:
        return await to_thread.run_sync(
            chat_service.get_history, conversation_id, principal.user_id
        )
    except KeyError:
        raise HTTPException(status_code=404, detail="会话不存在") from None
