...
async def _guard_conversation(conversation_id: str | None, user_id: str) -> None:
    ...
    try:
        await to_thread.run_sync(
            companion_service.ensure_access, conversation_id, user_id
        )
    except KeyError:
...
async def companion_chat(
    req: CompanionStreamRequest, user_id: str = CurrentUser
) -> StreamingResponse:
...
async def companion_history(
    ...
    try:
        return await to_thread.run_sync(
            companion_service.get_history, conversation_id, user_id
        )
    except KeyError:
        raise HTTPException(status_code=404, detail='会话不存在') from None
...
async def companion_conversations(
    ...
    return await to_thread.run_sync(companion_service.list_conversations, user_id)
