...
import { streamScience } from '@/api/science'
import { authFetch } from '@/utils/auth'

/**
 * 科普百科 Store：管理当前会话的消息列表、会话 ID、
 * 流式生成状态与错误信息（纯文本，无语音能力）。
 */
export const useScienceStore = defineStore('science', () => {
  /** 当前会话的消息列表（含用户消息与助手回复） */
  const messages = ref<ScienceMessage[]>([])
   ...
   */
  function makeMessage(role: 'user' | 'assistant', content: string): ScienceMessage {
    return reactive({
   ...
   * 3. 调用 streamScience 消费 SSE 流：
   *    - 首个事件携带 conversationId，用于保存/续聊；
   *    - 每收到 delta 就追加到助手消息内容；
  ...
  async function loadHistory(id: string): Promise<void> {
    const resp = await authFetch('/api/science/history/' + encodeURIComponent(id))
    if (!resp.ok) return
  ...
  async function loadConversations(): Promise<ScienceConversation[]> {
    const resp = await authFetch('/api/science/conversations')
    if (!resp.ok) return []
    return (await resp.json()) as ScienceConversation[]
  }

  /**
   * 重置会话状态：清空消息与会话 ID，复位流式状态和错误信息。
   */
  function reset() {
    messages.value = []
