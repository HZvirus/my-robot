...
export function useChat() {
  ...
  function handle(msg: WSMessage) {
    if (msg.type === 'status') {
      ...
      } else if (p.event === 'soothe' || p.event === 'escalate') {
    ...
    if (msg.type === 'action') {
      const last = messages.value[messages.value.length - 1]
      if (last && last.role === 'assistant') last.action = msg.payload
      return
    }
    if (msg.type === 'message') {
      streaming.active = false
  ...
  async function feedback(message: ChatMessage, score: 1 | -1) {
    ...
    try {
      await apiFeedback({ session_id: sessionId.value ?? '', message_id: message.id, score })
    } catch {
      // 静默失败
    }
  }
