...
- **SSE 流式已有**：`chat.py:51` `chat_stream` + `_SSE_HEADERS`（`:24`），`StreamingResponse`；triage/science/companion 同构。SSE 解析 `_parse_delta`（`llm_client.py:112`）。
- **中断**：`CancelledError` 时持久化 `interrupted=True`（`chat_service.py:60`、`triage_service.py:142`），但**只标记不恢复**，无法续写半截答案。
- **上下文剪裁**：仅截断（取最近 N），无智能剪裁。
- **无** ReAct/Plan-Execute/多 Agent、无 Token 成本管理；并发用 `httpx.AsyncClient` + `anyio.to_thread`（每次新建 client，`llm_client.py:101`，可优化为复用）。
...
### 4. Token 成本与效率
- [ ] Token 计数与预算（依赖 06 trace）；prompt 压缩、缓存复用（复用 `embedding.py` 缓存思路）
